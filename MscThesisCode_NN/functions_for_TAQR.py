...
def opdatering_final_torch(X, Xny, IX, Iy, Iex, Ih, Ihc, beta, Rny, K, n, xB, P, tau, i, bins, n_in_bin):
    # clean +inf and -inf from Xny
    Xny[Xny == torch.inf] = 1
    Xny[Xny == -torch.inf] = 1

    Xny = torch.vstack((Xny, X[n + i - 2, :]))

    Index = torch.arange(Xny[:-1, Iy].shape[0])
    ...
    minIL, Inmin = torch.min(torch.abs(Ih - Leav)), torch.argmin(torch.abs(Ih - Leav))

    if minIL == 0 and len(In) == n_in_bin:
        cB = (P < 0) + P * tau
        invXh = torch.linalg.inv(Xny[Ih, IX])
        g = -(P * (Xny[Ihc, IX] @ invXh[:, Inmin])).T @ cB
        h = torch.vstack((invXh[:, Inmin], -P * (Xny[Ihc, IX] @ invXh[:, Inmin]))) @ torch.sign(g)
        sigma = torch.zeros(n - K)
        hm = h[K:]
        xBm = xB[K:]
        xBm[xBm < 0] = 0
        tolerance = 1e-10
        sigma[hm > tolerance] = xBm[hm > tolerance] / hm[hm > tolerance]
        sigma[hm <= tolerance] = torch.inf
        alpha, q = torch.min(sigma), torch.argmin(sigma)
        ...
        P[q] = torch.sign(g) + (g == 0)
        Ih, _ = torch.sort(Ih)
        Ihc, IndexIhc = torch.sort(Ihc)
        P = P[IndexIhc]
        xBm = xB[K:]
        xBm = xBm[IndexIhc]
        xB[K:] = xBm
    ...
    # print the torch type of X and beta
    # print("X type: ", X.type())
    # print("beta type: ", beta.type())
    rny = X[n + i - 2, Iy] - X[n + i - 2, IX] @ beta
    Rny = torch.cat((Rny, rny.unsqueeze(0)))

    if rny < 0:
        P = torch.cat((P, torch.tensor([-1])))
        xB = torch.cat((xB, -rny.unsqueeze(0)))
    else:
        P = torch.cat((P, torch.tensor([1])))
        xB = torch.cat((xB, rny.unsqueeze(0)))
    Ihc = torch.cat((Ihc, torch.tensor([n])))

    if len(In) == n_in_bin:
        print("ever activated? ", i)
        i += 1
        Stay = torch.ones(len(Ihc), dtype=bool)
        Stay[Ihc == Leav] = False
        P = P[Stay]
        Ihc = Ihc[Stay]
        Xny = Xny[torch.sort(torch.hstack((Ih, Ihc)))[0], :]
        xBm = xB[K:]
        xBm = xBm[Stay]
        xB = xB[:-1]
        xB[K:] = xBm
...
def rq_initialiser_final(X, r, beta, n):
    ...
    # print("line 108, Ih:", Ih)
    # what if we here just chose Ihc as the complement of Index \ Ih
...
def rq_initialiser_final_torch(X, r, beta, n):
    Index = torch.arange(n)

    if torch.sum(r == 0) > len(beta):
        Lr0 = torch.sum(r == 0)
        rs, Irs = torch.sort(torch.abs(r))
        Irs = Irs[:Lr0]
        Xh = X[Irs, :]

        P, L, U = torch.linalg.lu(Xh) #, pivot=False)

        In = torch.arange(Lr0)
        rI = torch.zeros(Lr0 - len(beta), dtype=torch.long)
    ...
    if len(index_to_index) != len(Index):
        index_to_index = torch.cat((index_to_index, torch.zeros(len(Index) - len(index_to_index), dtype=torch.bool)))

    Ih = Index[index_to_index]
    Ihc = torch.tensor(list(set(Index[:len(r)].tolist()) - set(Ih.tolist())), dtype=torch.long)

    P = torch.sign(r[Ihc])
    r[torch.abs(r) < 1e-15] = 0
    xB = torch.hstack((beta, torch.abs(r[Ihc])))
...
def rq_simplex_alg_final_torch(Ih, Ihc, n, K, xB, Xny, IH, P, tau):
    invXh = torch.linalg.inv(Xny[Ih, :])

    cB = (P < 0) + P * tau
    cC = torch.vstack((torch.ones(K) * tau, torch.ones(K) * (1 - tau))).reshape(-1, 1)

    IB2 = -torch.matmul(P.reshape(-1, 1) * (torch.ones((1, K)) * Xny[Ihc, :]), invXh)
    g = torch.matmul(IB2.T, cB)
    d = cC - torch.vstack((g, -g)).reshape(-1, 1)
    d[torch.abs(d) < 1e-15] = 0
    d = d.flatten()

    md, s = torch.sort(d)
    s = s[md < 0]
    md = md[md < 0]
    c = torch.ones(len(s), dtype=torch.double)
    c[s > (K - 1)] = -1
    C = torch.diag(c)
    s[s > (K - 1)] -= K

    # print the type of InvXh, C, IB2
    # print("InvXh type: ", invXh.type())
    # print("C type: ", C.type())
    # print("IB2 type: ", IB2.type())
    h = torch.vstack([invXh[:, s], IB2[:, s]]) @ C

    alpha = torch.zeros(len(s))
    q = torch.zeros(len(s), dtype=torch.int)
    xm = xB[(K):]
    xm[xm < 0] = 0
    hm = h[K:, :]
    cq = torch.zeros(len(s))
    tol1 = 1e-12

    for k in range(len(s)):
        sigma = xm.clone()
        sigma[hm[:, k] > tol1] = xm[hm[:, k] > tol1] / hm[hm[:, k] > tol1, k]
        sigma[hm[:, k] <= tol1] = torch.inf
        alpha[k], q[k] = torch.min(sigma), torch.argmin(sigma)
        cq[k] = c[k]

    gain = md * alpha

    Mgain, IMgain = torch.sort(gain)
    CON = torch.inf
    j = 0
    ...
    if len(gain) == 0:
        ...
        while CON > 1e6 and j < len(s):
            ...
            IhMid = Ih.clone()
            shifter = 0
            # print type of  j-1+shifter, q, IMgain
            # print("IMgain: ", IMgain.type())
            # print(" q type: " ,q[IMgain[j - 1 + shifter]].type())
            # print("s type: ", s[IMgain[j - 1 + shifter]].type())
            IhMid[s[IMgain[j - 1 + shifter]]] = Ihc[q[IMgain[j - 1 + shifter]]]
            IhMid, _ = torch.sort(IhMid)
            ...
            if torch.min(torch.sum(torch.abs(IH - IhMid.T * torch.ones((1, IH.shape[1]))), dim=0)) == 0:
                CON = torch.inf
            else:
                CON = torch.linalg.cond(Xny[(IhMid.flatten()), :])

        s = s[IMgain[j - 1 + shifter]]
        q = q[IMgain[j - 1 + shifter]]
        cq = cq[IMgain[j - 1 + shifter]]
        alpha = alpha[IMgain[j - 1 + shifter]]
        IH = torch.hstack((IH, IhMid.T))
        h = h[:, IMgain[j - 1 + shifter]]
        gain = gain[IMgain[j - 1 + shifter]]
        md = md[IMgain[j - 1 + shifter]]
...
def rq_purify_final_torch(xB, Ih, Ihc, P, K, Xny, yny):
    invXh = torch.linalg.inv(Xny[Ih, :])
    
    xB_part1 = invXh @ yny[Ih]
    xB_part2 = yny[Ihc] - Xny[Ihc, :] @ invXh @ yny[Ih]
    xB = torch.hstack((xB_part1, xB_part2))
...
def rq_simplex_final_torch(X, IX, Iy, Iex, r, beta, n, tau, bins, n_in_bin):
    """
    rq_simplex_final_torch calculates the solution to an adaptive simplex
    algorithm for a quantile regression problem using PyTorch. The function uses
    knowledge of the solution at time t to calculate the solution at
    time t+1.

    (The rest of the introductory text is omitted for brevity)
    """
    ...
    Ld = torch.tensor([0])
    mx = 0
    T = torch.zeros(len(X[:, Iy]))
    Mx = torch.zeros(len(X[:, Iy]))
    N = torch.zeros((100, 2))
    N_num_of_simplex_steps = []
    N_size_of_the_design_matrix_at_time_k = []
    CON1 = torch.tensor([0])
    CON2 = torch.tensor([[0, 0]])
    BETA = beta.reshape(1, -1)
    ...
    H = torch.zeros((1, K))  # This is used to track solutions that the algorithm has visited
    ...
    # The function rq_initialiser_final initializes the parameters needed for
    # the simplex algorithm based on the solution given as input to the function
    xB, Ih, Ihc, P = rq_initialiser_final_torch(Xny[:, IX], r, beta, n)
    # pre allocate Re
    Re = torch.zeros(LX - n)
    # The while loop makes the simulation through the data set, in each step
    # either i or n is updated. n is the total number of elements in the
    ...
    while i + n < LX:
        ...
        Re[k] = torch.sum(P < 0) / n  # Reliability at time k
        mx = torch.min(xB[K:])  # The minimum of the basic solution, this should be larger than zero

        # Infeasible points are taken care of by rq_purify_final
        if j > 0 and mx < -tolmx:
            xB, P = rq_purify_final_torch(xB, Ih, Ihc, P, K, Xny[:, IX], Xny[:, Iy])
            mx = torch.min(xB[K:])
        ...
        BETA = torch.vstack((BETA, beta))

        # The design matrix and other variables needed for the simplex
        # procedure are updated
        Ih, Ihc, xB, Xny, Rny, P, n, i = opdatering_final_torch(
            X, Xny, IX, Iy, Iex, Ih, Ihc, beta, Rny, K, n, xB, P, tau, i, bins, n_in_bin
        )
        ...
        IH = Ih.reshape(-1, 1)  # The index set h is stored in each simplex step, this is done
                                ...
                                # equally good solutions
        # Numbers needed to perform the simplex step are calculated in rq_simplex_alg_final
        CON, s, q, gain, md, alpha, h, IH, cq = rq_simplex_alg_final_torch(
            Ih, Ihc, n, K, xB, Xny[:, IX], IH, P, tau
        )
        CON1 = torch.cat((CON1, torch.tensor(CON).unsqueeze(0)))  # The condition number of the matrix X(h(t+1)) is stored

        # The while loop continues until the optimal solution is reached or one
        # of the two stop criteria are violated. These are a maximal number of
        # simplex steps, and the condition number of the next solution.
        while gain <= 0 and md < 0 and j < 24 and CON < 1e6:
            GAIN = torch.cat((GAIN, gain.unsqueeze(0)))  # The gain for the step is stored
            j += 1  # The simplex counter is updated
            ...
            CON1 = torch.cat((CON1, torch.tensor(CON).unsqueeze(0)))  # The condition number is stored
        
        N_size_of_the_design_matrix_at_time_k.append(n)
        T[k] = time.time() - t  # CPU time used to update the solution

    N = torch.hstack([torch.tensor(N_num_of_simplex_steps), torch.tensor(N_size_of_the_design_matrix_at_time_k)])

    return N, BETA[1:], GAIN[1:], Ld, Rny, Mx, Re, CON1[1:], T

def one_step_quantile_prediction(X_input, Y_input, n_init, n_full, quantile = 0.5, already_correct_size = False):
    ...
    if m > 25:
        X_input = X_input[:, np.arange(1, m-2, 2)]
    else:
        X_input = X_input[:, np.arange(0,m-1, 2)]
        # print(X_input.shape, "X_input shape is being modified...")
        # X_input = X_input[:, np.arange(1,m-1, 3)]
    ...
    r_init = set_n_closest_to_zero(arr = residuals , n = p)
    ...
    Iy = p
    Iex = p + 1
    bins = np.array([-np.inf, np.inf]) # rolling
    # beta_init = quantile_fit.coef_
    tau = quantile
    n_in_bin = int(full_length) # 0.75*


    # call the function
    N, BETA, GAIN, Ld, Rny, Mx, Re, CON1, T = rq_simplex_final(X_full, IX, Iy, Iex, r_init, beta_init, n_init, tau, bins, n_in_bin)
    # find the actual prediction
    # print(BETA.shape, "BETA SHAPE")
    # print(X_input.shape, "X_input full shape")
    # print(X_input[(n_init+1):(n_full), :].shape, "X_input shape") 
    y_pred = np.sum((X_input[(n_init+2):(n_full), :] * BETA[1:,:]), axis = 1) # TODO WHETHER IT IS +1, or 2 here or minus, should def. be checked
    y_actual = Y_input[(n_init):(n_full-2)]
...
def one_step_quantile_prediction_torch(X_input, Y_input, n_init, n_full, quantile=0.5):
    ...
    assert n_init <= n_full - 2, "n_init must be less than n_full"  # should it be equal == ?
    ...
    # calculate residuals
    quantile_fit = model_QR.fit(X_for_residuals.detach().numpy(), Y_for_residuals.detach().numpy())
    Y_predict = quantile_fit.predict(X_for_residuals.detach().numpy())
    residuals = Y_for_residuals - torch.tensor(Y_predict, requires_grad = True)
    r_init = set_n_smallest_to_zero_torch(arr=(residuals) , n=p)

    # get the data ready
    # print("X shape: ", X.shape, "Y shape: ", Y.shape, "random choice shape: ", np.random.choice([1,1], size=n_full).shape)
    X_full = torch.column_stack((torch.tensor(X), torch.tensor(Y), torch.randint(1, 2, size=(n_full,))))
    IX = torch.arange(p)
    Iy = p
    Iex = p + 1
    bins = torch.tensor([-torch.inf, torch.inf])  # rolling
    beta_init = torch.tensor(quantile_fit.coef_)
    tau = quantile
    n_in_bin = int(0.75 * full_length)

    # call the function
    N, BETA, GAIN, Ld, Rny, Mx, Re, CON1, T = rq_simplex_final_torch(X_full, IX, Iy, Iex, r_init, beta_init, n_init, tau, bins, n_in_bin)

    # find the actual prediction
    # print(BETA.shape, "BETA SHAPE")
    y_pred = torch.sum(((X_input[(n_init+1):(n_full-1), :]) * BETA), dim=1)  # TODO WHETHER IT IS +1, or 2 here or minus, should def. be checked
    y_actual = (Y_input[(n_init+1):(n_full-1)])
    y_actual_quantile = torch.quantile(y_actual, quantile)
